...
class DICGCritic(nn.Module):
    """Centralized critic that uses the DICG encoder (CTDE style)."""
    ...
    def forward(self, batch, t=None):
        ...
        values = self.aggregator(embeddings)
        values = values.view(bs, max_t, self.n_agents, 1)
        return values
    ...
    def _build_inputs(self, batch, t=None):
        ...
        # observations (optionally concatenated across agents)
        ...
        if self.args.obs_last_action:
            if t == 0:
                ...
                last_actions = (
                    last_actions.view(bs, max_t, 1, -1)
                    .repeat(1, 1, self.n_agents, 1)
                )
