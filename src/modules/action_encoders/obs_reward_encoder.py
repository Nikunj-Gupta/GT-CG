...
class ObsRewardEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.obs_encoder_avg = nn.Sequential(
            nn.Linear(
                self.obs_dim + self.n_actions * (self.n_agents - 1),
                args.state_latent_dim * 2,
            ),
            nn.ReLU(),
        ...
        self.action_encoder = nn.Sequential(
            nn.Linear(self.n_actions, args.state_latent_dim * 2),
            nn.ReLU(),
    ...
    def predict(self, obs, actions):
        ...
        return no_pred_avg.view(-1, self.n_agents, self.obs_dim), r_pred_avg.view(
            -1, self.n_agents, 1
        )
    ...
    def other_actions(self, actions):
        ...
        for i in range(self.n_agents):
            ...
            for j in range(self.n_agents):
                if i != j:
                    other_actions_i.append(actions[:, j])
            other_actions_i = th.cat(other_actions_i, dim=-1)
            other_actions.append(other_actions_i)

        other_actions = (
            th.stack(other_actions, dim=1)
            .contiguous()
            .view(-1, (self.n_agents - 1) * self.n_actions)
        )
