...
class CASECMAC(object):
    def __init__(self, scheme, groups, args):
        ...
        delta_input_length = (
            2 * self.args.pair_rnn_hidden_dim
            if self.independent_p_q
            else 2 * self.args.rnn_hidden_dim
        )
        ...
        self.eye2 = th.eye(self.n_agents).bool().to(args.device)
        self.edges_from = (
            th.tensor([[i] * self.n_agents for i in range(self.n_agents)])
            .view(-1)
            .unsqueeze(0)
            .to(args.device)
        )
        self.edges_to = (
            th.tensor([[i for i in range(self.n_agents)] * self.n_agents])
            .view(-1)
            .unsqueeze(0)
            .to(args.device)
        )
        ...
        self.p_action_repr = (
            th.cat([input_i, input_j], dim=-1)
            .view(self.n_actions * self.n_actions, -1)
            .t()
            .unsqueeze(0)
        )
    ...
    def MaxSum_faster(self, x, adj, q_ij, atten_ij=None, available_actions=None, k=5):
        adj[:, self.eye2] = 0.0
        num_edges = int(adj[0].sum(-1).sum(-1))
        if num_edges == 0:
            if available_actions is not None:
                x = x.masked_fill(available_actions == 0, -99999999)
            return x
        edges_from = self.edges_from.repeat(x.shape[0], 1)[
            adj.view(-1, self.n_agents**2) == 1
        ].view(-1, num_edges)
        edges_to = self.edges_to.repeat(x.shape[0], 1)[
            adj.view(-1, self.n_agents**2) == 1
        ].view(-1, num_edges)
        nodes = th.cat([edges_from, edges_to], dim=1)
        ...
        if available_actions is not None:
            x = x.masked_fill(available_actions == 0, -99999999)
            available_actions_new = th.gather(
                available_actions,
                dim=1,
                index=nodes.unsqueeze(-1).repeat(1, 1, self.n_actions),
            ).view(-1, 2, num_edges, self.n_actions)

        for _ in range(k):
            q_left_down_sum = torch_scatter.scatter_add(
                src=r_down_left.view(-1, 2 * num_edges, self.n_actions),
                index=nodes,
                dim=1,
                dim_size=self.n_agents,
            )
            q_left_down_sum += x
            q_left_down = th.gather(
                q_left_down_sum,
                dim=1,
                index=nodes.unsqueeze(-1).repeat(1, 1, self.n_actions),
            ).view(-1, 2, num_edges, self.n_actions)
            ...
            if available_actions is not None:
                q_left_down -= (
                    (q_left_down * available_actions_new).sum(-1, keepdim=True)
                    / available_actions_new.sum(-1, keepdim=True)
                )
            else:
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False):
        ...
        agent_outputs = self.max_sum(
            ep_batch,
            t_ep,
            f_i=f_i,
            delta_ij=delta_ij,
            q_ij=q_ij,
            his_cos_sim=his_cos_sim.detach(),
            atten_ij=atten_ij,
        )
    ...
    def forward(self, ep_batch, t):
        ...
        agent_actions = ep_batch["actions"][:, t]
        f_i_gather = th.gather(f_i, index=agent_actions, dim=-1)

        agent_actions_gather_i = (
            agent_actions.unsqueeze(dim=2)
            .unsqueeze(dim=-1)
            .repeat(1, 1, self.n_agents, 1, self.n_actions)
        )
        agent_actions_gather_j = (
            agent_actions.unsqueeze(dim=1)
            .unsqueeze(dim=-2)
            .repeat(1, self.n_agents, 1, 1, 1)
        )
        ...
        if self.construction_attention:
            agent_outs = (
                f_i_gather.squeeze(dim=-1).mean(dim=-1)
                + (edge_attr * atten_ij).sum(dim=-1).sum(dim=-1) * self.p_lr
            )
        else:
            agent_outs = (
                f_i_gather.squeeze(dim=-1).mean(dim=-1)
                + edge_attr.sum(dim=-1).sum(dim=-1) / self.n_agents / (self.n_agents - 1) * self.p_lr
            )
    ...
    def max_sum(
        ...
        his_cos_sim=None,
        atten_ij=None,
        target_delta_ij=None,
        target_q_ij=None,
        target_his_cos_sim=None,
        target_atten_ij=None,
    ):
        x, adj, _, q_ij = self.construction(
            f_i,
            delta_ij,
            q_ij,
            his_cos_sim,
    ...
    def calculate(self, ep_batch, t):
        ...
        agent_outs, self.hidden_states = self.agent(
            agent_inputs, self.hidden_states, self.action_repr
        )
        agent_outs = agent_outs.view(self.bs, self.n_agents, self.n_actions)
        f_i = agent_outs.clone()

        if self.independent_p_q:
            self.p_hidden_states = self.p_agent.h_forward(
                agent_inputs, self.p_hidden_states
            ).view(self.bs, self.n_agents, -1)
        else:
        ...
        f_i_expand_j = (
            f_i.unsqueeze(dim=1)
            .unsqueeze(dim=-2)
            .repeat(1, self.n_agents, 1, self.n_actions, 1)
        )
        f_i_expand_i = (
            f_i.unsqueeze(dim=2)
            .unsqueeze(dim=-1)
    ...
    def _calculate_attention(self, hidden_states):
        atten_i = (
            self.atten_key(hidden_states.view(-1, self.args.rnn_hidden_dim))
            .view(self.bs, self.n_agents, -1)
            .unsqueeze(2)
            .repeat(1, 1, self.n_agents, 1)
        )
        atten_j = (
            self.atten_query(hidden_states.view(-1, self.args.rnn_hidden_dim))
            .view(self.bs, self.n_agents, -1)
            .unsqueeze(1)
            .repeat(1, self.n_agents, 1, 1)
        )

        atten_ij = (atten_i * atten_j).sum(-1).view(self.bs, -1)
    ...
    def _calculate_delta(self, hidden_states):
        ...
        if self.use_action_repr:
            ...
            f_ij = (
                th.bmm(key, self.p_action_repr.repeat(self.bs, 1, 1))
                / self.args.action_latent_dim
                / 2
            )
        else:
        ...
        f_ij = (f_ij + f_ij.permute(0, 2, 1, 4, 3).detach()) / 2.0
    ...
    def construction(
        ...
        available_actions_i = (
            available_actions.detach()
            .unsqueeze(dim=2)
            .repeat(1, 1, self.n_agents, 1)
            .view(-1, self.n_agents * self.n_agents, self.n_actions)
        )
        available_actions_j = (
            available_actions.detach()
            .unsqueeze(dim=1)
        ...
        available_actions_ij = (
            available_actions_i.unsqueeze(dim=-1).repeat(1, 1, 1, self.n_actions)
            * available_actions_j
        ).view(-1, self.n_agents * self.n_agents, self.n_actions * self.n_actions)
        x = f_i.clone()
        ...
        if self.full_graph:
            ...
            if self.random_graph:
                ...
                if target_q_ij is not None:
                    indicator = self._variance(
                        target_q_ij.detach().view(
                            -1, self.n_agents * self.n_agents, self.n_actions, self.n_actions
                        ),
                        available_actions_j,
                    )
                ...
                indicator = (
                    target_his_cos_sim.view(-1, self.n_agents * self.n_agents)
                    if target_his_cos_sim is not None
                    else his_cos_sim.view(-1, self.n_agents * self.n_agents)
                )
            elif self.construction_attention:
                indicator = (
                    target_atten_ij.detach().view(-1, self.n_agents * self.n_agents)[0]
                    if target_atten_ij is not None
                    else atten_ij.detach().view(-1, self.n_agents * self.n_agents)[0]
                )
            else:
                # Default fallback when no construction mode is selected.
                indicator = (
                    delta_ij.detach()
                    .view(
                        -1,
            ...
            adj_tensor = indicator.masked_fill(
                self.wo_diag.repeat(self.bs, 1, 1).view(-1, self.n_agents * self.n_agents) == 0,
                -99999999,
            )
            adj_tensor_topk = th.topk(
                adj_tensor,
                int(self.n_agents * self.n_agents * self.args.threshold // 2 * 2),
                dim=-1,
            )[1]
    ...
    def init_hidden(self, batch_size):
        ...
        if self.independent_p_q:
            self.p_hidden_states = self.p_agent.init_hidden().unsqueeze(0).expand(
                batch_size, self.n_agents, -1
            )
        else:
            self.p_hidden_states = self.agent.init_hidden().unsqueeze(0).expand(
                batch_size, self.n_agents, -1
            )
    ...
    def parameters(self):
        if self.construction_attention:
            param = (
                ...
                + list(self.delta.parameters())
                + list(self.atten_query.parameters())
                + list(self.atten_key.parameters())
            )
        elif self.independent_p_q:
            param = (
                list(self.agent.parameters())
                + list(self.p_agent.parameters())
                + list(self.delta.parameters())
            )
        else:
    ...
    def load_models(self, path):
        ...
        self.delta.load_state_dict(
            th.load("{}/delta.th".format(path), map_location=lambda storage, loc: storage)
        )
        self.action_encoder.load_state_dict(
            th.load("{}/action_encoder.th".format(path), map_location=lambda storage, loc: storage)
        )

        self.action_repr = th.load(
            "{}/action_repr.pt".format(path), map_location=lambda storage, loc: storage
        ).to(self.args.device)
        self.p_action_repr = th.load(
            "{}/p_action_repr.pt".format(path), map_location=lambda storage, loc: storage
        ).to(self.args.device)
    ...
    def action_repr_forward(self, ep_batch, t):
        return self.action_encoder.predict(
            ep_batch["obs"][:, t], ep_batch["actions_onehot"][:, t]
        )
